...
class AutomationSyncReadCheckpointBackend implements SyncReadCheckpointBackend {
  readonly consumer = 'automation' as const;
  ...
  > {
    const { rows } = await this.pool.query<SyncReadCheckpointStorageRow & pg.QueryResultRow>(
      `INSERT INTO automation_sync_read_consumer_checkpoint (
       ...
       VALUES ($1, 'automation', $2, $3::numeric, $4, $5, $6, $7, $8, $9, $10, now())
       ...
       DO UPDATE SET
         ...
         updated_at = now()
       WHERE automation_sync_read_consumer_checkpoint.applied_cursor IS NULL
          OR (
            ...
            AND (
              ...
              OR (
                EXCLUDED.applied_cursor = automation_sync_read_consumer_checkpoint.applied_cursor
                AND EXCLUDED.payload_digest = automation_sync_read_consumer_checkpoint.payload_digest
                AND (
                  (
                    EXCLUDED.source_as_of_ms > automation_sync_read_consumer_checkpoint.source_as_of_ms
                    AND EXCLUDED.last_observed_at_ms >= automation_sync_read_consumer_checkpoint.last_observed_at_ms
                    AND EXCLUDED.last_applied_at_ms = automation_sync_read_consumer_checkpoint.last_applied_at_ms
                  )
                  OR (
                    EXCLUDED.source_as_of_ms = automation_sync_read_consumer_checkpoint.source_as_of_ms
                    AND EXCLUDED.last_observed_at_ms = automation_sync_read_consumer_checkpoint.last_observed_at_ms
                    AND EXCLUDED.fresh_until_ms = automation_sync_read_consumer_checkpoint.fresh_until_ms
                    AND EXCLUDED.last_applied_at_ms = automation_sync_read_consumer_checkpoint.last_applied_at_ms
                  )
                )
              )
...
export function createAutomationSyncReadConsumerCheckpointStore(
  pool: pg.Pool,
  executionTarget: DeploymentTarget,
): SyncReadConsumerCheckpointStore {
  return new SyncReadConsumerCheckpointStore({
    executionTarget,
    consumer: 'automation',
    backend: new AutomationSyncReadCheckpointBackend(pool),
  });
}
