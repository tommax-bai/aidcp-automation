...
export type FacebookCommentOutcome =
  ...
  | 'comment_rejected' // 平台已拒绝：确定未上墙、终局（真机坐实「已拒绝/查看反馈」行）。绝不染绿、绝不去重，留人工
...
export class FacebookCommentAuditStore {
  ...
  async init(): Promise<void> {
    ...
    await ensureCapabilitySchema(this.pool, {
      capability: 'facebook_comment_audit',
      sinceVersion: '0035_facebook_comment_audit',
      ddl: [FACEBOOK_COMMENT_AUDIT_SCHEMA_SQL],
    });
  }
