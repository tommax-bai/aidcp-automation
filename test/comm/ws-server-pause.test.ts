...
test('暂停后普通指令被丢弃、session.end/ui.push_snapshot/captcha assist 恢复命令必达、恢复后续发', async () => {
  ...
  const taskAcquire = makeEnvelope('task.acquire', 'c5-task', 0, {
  ...
  const publish = makeEnvelope('xiaohongshu.publish.command', 'c6', 0, { taskId: 'task-publish-1', recordId: 1, seq: 1, kind: 'submit_publish', params: {} });
