/**
 * 验收用例 AC-PROTO-* — 边-云协议契约一致性（云端侧）
 *
 * 守护点：aidcp-cloud/src/comm/protocol.ts 与 aidcp-edge/src/comm/protocol.ts 必须是同一份契约。
 *   本测试用 `Record<MessageType, true>` 穷举全部消息类型——若任一端增删/改名 MessageType
 *   而未同步，该端 `npm run typecheck` 立即失败（缺 key 或多 key）；运行时再校验版本号、
 *   消息总数与信封往返。边缘有一份内容完全一致的对照测试
 *   （aidcp-edge/test/acceptance/protocol-contract.test.ts）。
 *
 * 环境层级：离线 / 逻辑级（无外部依赖）。
 */
...
const ALL_MESSAGE_TYPES: Record<MessageType, true> = {
  ...
  'wechat_channels.inbox.auth.status': true, 'wechat_channels.inbox.sync.batch': true, 'wechat_channels.inbox.sync.ack': true,
  'wechat_channels.inbox.reply.result': true, 'wechat_channels.inbox.reply.result.ack': true,
  'wechat_channels.inbox.reply.reconcile': true, 'wechat_channels.inbox.reply.reconcile.result': true,
  'wechat_channels.inbox.sync.request': true,
  'wechat_channels.inbox.reply.send': true, 'wechat_channels.inbox.auth.reopen': true,
...
describe('AC-PROTO 协议契约一致性（cloud）', () => {
  it('AC-PROTO-01 协议版本为 2', () => {
    assert.equal(PROTOCOL_VERSION, 2);
  });

  it('AC-PROTO-02 消息类型总数为 108（增删消息须同步冻结契约 + 本断言）', () => {
